...
export default function ViewContractPerson() {
  ...
  function GetData() {
    ...
    get(child(dbRef, "financeiravip/contractsPersons")).then((snapshot) => {
      if (snapshot.exists()) {
  ...
  async function RemoveData(idContrato: string) {
    try {
      await remove(ref(db, `financeiravip/contractsPersons/${idContrato}`));
        ...
        <TableBody>
          {Object.values(contractData.contrato).map((contrato, index) => (
            <TableRow key={index}>
              ...
              <TableCell>{contrato.age}</TableCell>
              <TableCell>
